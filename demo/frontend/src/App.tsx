...
const App = () => {
  ...
  const [previousSessionPartId, setPreviousSessionPartId] = useState<
    string | null
  >(null);
  ...
  useEffect(() => {
    const lifecycleManager = session.getLifecycleManager();

    const updateUserSession = () => {
      setUserSessionId(session.getUserSessionId());
      setPreviousUserSessionId(session.getPreviousUserSessionId());
      const attrs = lifecycleManager.getUserSessionAttributes();
    ...
    const updateSessionPart = () => {
      ...
      setPreviousSessionPartId(sessionPartManager.getPreviousSessionPartId());
      const attrs = lifecycleManager.getUserSessionAttributes();
      setUserSessionPartNumber(attrs?.['emb.user_session_part_number'] ?? null);
      const hrStart = sessionPartManager.getSessionPartStartTime();
      setPartStartTs(
        hrStart === null ? null : hrStart[0] * 1000 + hrStart[1] / 1_000_000,
      );
      const spanAttrs = sessionPartManager.getSessionPartSpan()?.attributes;
    ...
    const unsubscribeUserStart =
      session.addUserSessionStartedListener(updateUserSession);
    const unsubscribeUserEnd =
      session.addUserSessionEndedListener(updateUserSession);
    const unsubscribePartStart =
      sessionPartManager.addSessionPartStartedListener(handlePartStart);
    const unsubscribePartEnd =
      sessionPartManager.addSessionPartEndedListener(handlePartEnd);

    // user / page APIs have no listener hook and SDK state can change
    // cross-tab (localStorage); poll to keep the whole panel live. Also
    // refresh immediately on focus / visibility gain so the display is
    // current the moment this tab becomes engaged again.
    //
    // Demo-only: polling at 500ms is fine for a debugging UI. Customer apps
    // should not need this; they receive session/part lifecycle events via
    // the public listener APIs (`session.addUserSessionStartedListener`,
    // `addSessionPartStartedListener`, etc.) and access the latest user /
    // page values lazily on render. The user/page managers intentionally do
    // not expose listener APIs today; if a customer use case appears, the
    // right answer is to add proper listeners rather than adopt this poll.
