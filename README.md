...
If you wish to sample the amount of traces generated by the sdk, you can do so by

1. Passing a `spanSampler` parameter implementing your custom sampling when initializing the SDK. This allows
   for complete control on the sampling strategy.
2. passing a `spanSamplingRatio` with a value between 0 and 1. This will use our default sampling strategy targeting
   a percentage of sampling equal to the value of `spanSamplingRatio`. This is a simple way to control the amount of
   traces

> [!NOTE]
> The session span is referenced by other telemetry (eg. logs have a `session.id` attribute), so if you sample the
> session span not to be included, you should use the same logic to sample the other spans and logs, or be aware of the
> possible inconsistent data that will be exported.
...
const result = sdk.initSDK({
  ...
  appVersion: "YOUR_APP_VERSION",
  // sampler
  spanSampler: new CustomSampler(),
  // OR
  spanSamplingRatio: 0.5, // 50% of traces will be sampled
});
```
