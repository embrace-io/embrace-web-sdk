...
import { NoOpSpanSessionManager, session } from '../api-sessions/index.ts';
import {
  trace as embtrace,
  NoOpTraceManager,
  ProxyTraceManager,
...
const getLastSessionExportedSpans = async (
  ...
  expect(sessionScopeSpan['scope']).to.deep.equal({
    name: 'embrace-web-sdk-user-sessions',
  });
...
describe('initSDK', () => {
  ...
  it('should register all global managers', async () => {
    ...
    expect(session)
      .to.have.property('setGlobalSessionManager')
      .that.is.a('function');
    expect(session)
      .to.have.property('getSpanSessionManager')
      .that.is.a('function');
  ...
  describe('communication with Embrace', () => {
    ...
    it('should apply limits on the number of attributes of an individual span', async () => {
      ...
      expect(bypassKeys).to.have.members(['browser.url.full', 'session.id']);
    });
...
describe('isolated instances', () => {
  ...
  it('should not register any provider globally', async () => {
    ...
    session.getSpanSessionManager().endSessionPartInternal('inactivity');
    session.getSpanSessionManager().startSessionPartInternal('activity');
    session.getSpanSessionManager().endSessionPartInternal('inactivity');
  ...
  it('should allow each instance to emit its own telemetry from the provided managers', async () => {
    ...
    const checkInstanceTelemetry = async (
      ...
      internalSessionManager.startSessionPartInternal('activity');
      internalSessionManager.endSessionPartInternal('inactivity');
      instrumentation.emit();
      ...
      // Two emb-session spans are expected per instance: the init part is
      // ended explicitly, then a fresh activity part is opened and ended.
