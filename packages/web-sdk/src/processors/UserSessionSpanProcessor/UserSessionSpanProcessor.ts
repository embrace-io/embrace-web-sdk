...
export class UserSessionSpanProcessor implements SpanProcessor {
  ...
  public onEnd(span: ReadableSpan): void {
    try {
      ...
      if (span.attributes['session.id'] === undefined) {
        if (ids.sessionIdOverride !== null) {
          span.attributes['session.id'] = ids.sessionIdOverride;
        } else {
          span.attributes['session.id'] = userSessionId;
        }
      }
      if (span.attributes['session.previous_id'] === undefined) {
        span.attributes['session.previous_id'] = userSessionPreviousId;
      }
      ...
      if (span.attributes['session.previous_id'] === undefined) {
        span.attributes['session.previous_id'] = '';
      }
      this._diag.error(
