...
import { generateUUID, SafeStorage } from '../../utils/index.ts';
import {
...
export class EmbraceUserManager implements UserManagerInternal {
  ...
  public setEmbraceUserId(userId: string) {
    ...
    this._storage.write(EMBRACE_USER_ID_STORAGE_KEY, userId);
  }

  public clearEmbraceUserId() {
    this._embraceUserId = null;
    this._storage.remove(EMBRACE_USER_ID_STORAGE_KEY);
  }

  private _initialSetup() {
    const embraceUserId = this._storage.read(EMBRACE_USER_ID_STORAGE_KEY);
    if (!embraceUserId) {
  ...
  public getUserId(): string | null {
    return this._storage.read(EMBRACE_EXTERNAL_USER_ID_KEY);
  }

  public setUserId(userId: string): void {
    this._storage.write(EMBRACE_EXTERNAL_USER_ID_KEY, userId);
  }

  public clearUserId(): void {
    this._storage.remove(EMBRACE_EXTERNAL_USER_ID_KEY);
  }
}
