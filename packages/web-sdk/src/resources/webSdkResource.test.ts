...
describe('webSdkResource', () => {
  ...
  beforeEach(() => {
    ...
    storage = new EmbraceStorage(inMemoryStorage, diagLogger);
  });
  ...
  describe('getWebSDKResource', () => {
    ...
    it('should handle its storage throwing errors', () => {
      const resource = getWebSDKResource({
        diagLogger,
        appVersion: '1.0.0',
        pageSessionStorage: new EmbraceStorage(
          new FailingStorage(),
          diagLogger,
        ),
      });
      ...
      // Read fails: EmbraceStorage warns. Write fails: EmbraceStorage flips disabled
      // and emits one error.
