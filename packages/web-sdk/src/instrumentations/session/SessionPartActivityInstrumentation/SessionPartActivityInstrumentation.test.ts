...
describe('SessionPartActivityInstrumentation', () => {
  ...
  // Pulls reasons from the spies in the same order startSessionPart /
  // endSessionPartInternal were invoked. Filters undefined to keep test
  // assertions stable when callers omit the reason argument.
  const startReasons = (): Array<SessionPartStartReason | undefined> =>
    startSpy.getCalls().map((c) => c.args[0]);
  const endReasons = (): SessionPartEndReason[] =>
    endSpy
      .getCalls()
      .map((c) => c.args[0] as SessionPartEndReason)
      // Real EmbraceUserSessionManager.endSessionPart() forwards to
      // endSessionPartInternal('manual') only when a part is active; spurious
      // no-op calls are filtered out by the manager itself, so the spy only
      // sees actual end-with-effect invocations.
      .filter((r): r is SessionPartEndReason => r !== undefined);
  ...
  beforeEach(() => {
    ...
    startSpy = sinon.spy(userSessionManager, 'startSessionPart');
    endSpy = sinon.spy(userSessionManager, 'endSessionPartInternal');
  ...
  it('starts a new part with reason activity after an inactivity-killed part', () => {
    ...
    void expect(userSessionManager.getSessionPartId()).to.not.be.null;
  ...
  it('clears the part-inactivity timer when a part ends through another path', () => {
    ...
    userSessionManager.endSessionPartInternal('user_session_ended');
