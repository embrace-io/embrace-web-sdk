...
export class SpanSessionBrowserActivityInstrumentation extends EmbraceInstrumentationBase {
  ...
  public enable(): void {
    ...
    this._unsubPartStarted = this.sessionManager.addSessionPartStartedListener(
      this._onPartStarted,
    );
    this._unsubPartEnded = this.sessionManager.addSessionPartEndedListener(
      this._onPartEnded,
    );
    ...
    this._target.addEventListener('pagehide', this._onEngagementChange);
    if (this.sessionManager.getSessionPartId() !== null) {
      this._startPartInactivityTimer();
    }
  }
  ...
  public disable(): void {
    ...
    this._target.removeEventListener('pagehide', this._onEngagementChange);
  ...
  private readonly _onActivity = (): void => {
    try {
      ...
      if (this.sessionManager.getSessionPartId() === null) {
        this.sessionManager.startSessionPartInternal('activity');
        return;
      }
      this._startPartInactivityTimer();
    } catch (e) {
  ...
  private readonly _onEngagementChange = (): void => {
    try {
      ...
      const active = this.sessionManager.getSessionPartId() !== null;
      if (!engaged && active) {
        this._diag.debug('tab disengaged; ending current part');
        this.sessionManager.endSessionPartInternal('visibility_change');
        return;
      }
      if (engaged && !active) {
        this._diag.debug('tab engaged; starting new part');
        this.sessionManager.startSessionPartInternal('visibility_change');
        return;
      }
      if (engaged && active) {
        this._startPartInactivityTimer();
  ...
  private _startPartInactivityTimer(): void {
    this._clearPartInactivityTimer();
  ...
  private readonly _onPartInactivity = (): void => {
    ...
    try {
      ...
      this.sessionManager.endSessionPartInternal('inactivity');
    } catch (e) {
