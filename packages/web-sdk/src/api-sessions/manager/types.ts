...
export interface SpanSessionManager {
  ...
  /**
   * @deprecated Will be removed in a future version, no-op: per spec 1.4
   * there is no public start API. Foreground engagement (visibility,
   * focus, input) drives session-part starts internally.
   */
  ...
  /**
   * Overrides the `session.id` attribute on spans and logs. Pass `null` to
   * clear the override and fall back to `emb.user_session_id`. Does not
   * affect `emb.user_session_id` itself.
   *
   * Empty or whitespace-only strings and non-string non-null values are
   * rejected with a warning. Accepted values are truncated by the limit
   * manager.
   */
...
export type UserSessionEndReason =
  ...
  // Reserved per spec §5. The web SDK does not emit this value: inactivity
  // is detected lazily on the next part start, by which time the prior
  // part's span has already been finalized and exported. Kept in the
  // union to match the spec for consumers narrowing on the type.
  | 'inactivity';
