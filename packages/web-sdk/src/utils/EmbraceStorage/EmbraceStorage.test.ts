...
describe('EmbraceStorage', () => {
  ...
  describe('without namespace', () => {
    ...
    it('getStorageEventKey returns the logical key unchanged', () => {
      const storage = new EmbraceStorage(inMemoryStorage, diag);
  ...
  describe('with namespace', () => {
    ...
    it('keys() returns logical keys; length and key(i) reflect the namespaced view', () => {
      const storage = new EmbraceStorage(inMemoryStorage, diag, 'prefix');
    ...
    it('getStorageEventKey returns the namespaced physical key', () => {
      const storage = new EmbraceStorage(inMemoryStorage, diag, 'app123');
  ...
  describe('error handling', () => {
    ...
    it('propagates TypeError from setItem without disabling writes', () => {
      ...
      expect(() => storage.setItem('k', 'v')).to.throw(TypeError);
      expect(storage.isWriteDisabled()).to.equal(false);
      expect(diag.getErrorLogs()).to.have.lengthOf(0);

      expect(() => storage.setItem('k2', 'v2')).to.throw(TypeError);
      expect(storage.isWriteDisabled()).to.equal(false);
      expect(diag.getErrorLogs()).to.have.lengthOf(0);
    });
